...
// Always available to any authenticated user — a missing Seed or
// incomplete onboarding should never hide navigation.
const links = [
  { label: "Dashboard", to: "/dashboard" },
  { label: "Seeds", to: "/seeds" },
  { label: "Grove", to: "/grove" },
          ...
          >
            <div className="flex flex-col gap-1 px-6 py-4">
              ...
              <Link
                to="/grove"
                onClick={() => setOpen(false)}
                className="mt-2 flex items-center gap-2 rounded-lg border-t border-border px-3 pt-4 text-sm font-medium text-ink-soft transition-colors hover:text-ink"
              >
                <span className="flex h-6 w-6 items-center justify-center rounded-full bg-accent text-[10px] font-semibold text-white">
                  {initials}
                </span>
                View my Grove
              </Link>
              <button
                type="button"
                onClick={handleSignOut}
                className="flex items-center gap-2 rounded-lg px-3 py-2.5 text-left text-sm font-medium text-ink-soft transition-colors hover:bg-accent-soft hover:text-ink"
              >
                <LogOut className="h-3.5 w-3.5" strokeWidth={2} />
                Sign Out
              </button>
